  ...
  const runBacktestInternal = async (params: any) => {
    ...
    try {
      const queryParams = new URLSearchParams({
        symbol: String(params.symbol || 'NIFTY'),
        timeframe: String(params.timeframe || '5 Min'),
        strategy: String(params.strategy || 'ema9_rsi_momentum'),
        start_date: String(params.startDate || params.start_date || ''),
        end_date: String(params.endDate || params.end_date || ''),
        initial_capital: String(params.initialCapital || params.initial_capital || '100000'),
        quantity: String(params.quantity ?? 65),
        stoploss_pct: String(params.stoploss_pct ?? 0.6),
        target_pct: String(params.target_pct ?? 2.5),
        enable_ema_filter: String(params.enableEmaFilter ?? params.enable_ema_filter ?? false),
        enable_volume_filter: String(params.enableVolumeFilter ?? params.enable_volume_filter ?? false),
        enable_adx_filter: String(params.enableAdxFilter ?? params.enable_adx_filter ?? false),
        enable_vwap_filter: String(params.enableVwapFilter ?? params.enable_vwap_filter ?? false),
        enable_rsi_filter: String(params.enableRsiFilter ?? params.enable_rsi_filter ?? false),
        enable_squeeze_filter: String(params.enableSqueezeFilter ?? params.enable_squeeze_filter ?? true),
        enable_extension_filter: String(params.enableExtensionFilter ?? params.enable_extension_filter ?? false),
        enable_cpr_filter: String(params.enableCprFilter ?? params.enable_cpr_filter ?? false),
        enable_aggression_filter: String(params.enableAggressionFilter ?? params.enable_aggression_filter ?? false),
        donchian_period: String(params.donchian_period ?? params.donchianPeriod ?? 10),
        trailing_sl: String(params.trailing_sl ?? params.trailingSl ?? true),
        enable_pyramiding: String(params.enable_pyramiding ?? params.enablePyramiding ?? true),
        scale_pct: String(params.scale_pct ?? params.scalePct ?? 0.2),
        max_scales: String(params.max_scales ?? params.maxScales ?? 2),
        max_daily_loss_pct: String(params.max_daily_loss_pct ?? params.maxDailyLossPct ?? 3),
        max_daily_trades: String(params.max_daily_trades ?? params.maxDailyTrades ?? 0),
        trail_trigger: String(params.trail_trigger ?? params.trailTrigger ?? 0.5),
        trail_offset: String(params.trail_offset ?? params.trailOffset ?? 0.35)
      });
